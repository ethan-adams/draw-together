...
func (ec *executionContext) resolveEntity(
	...
	case "Board":
		...
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "Board": %w`, err)
		}
		switch resolverName {
		...
		case "findBoardByID":
			...
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findBoardByID(): %w`, err)
			}
			entity, err := ec.Resolvers.Entity().FindBoardByID(ctx, id0)
			if err != nil {
				return nil, fmt.Errorf(`resolving Entity "Board": %w`, err)
			}
...
func entityResolverNameForBoard(ctx context.Context, rep EntityRepresentation) (string, error) {
	...
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"id\" for Board", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if allNull {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to all null value KeyFields for Board", ErrTypeNotFound))
			break
		}
		return "findBoardByID", nil
	}
	return "", fmt.Errorf("%w for Board due to %v", ErrTypeNotFound,
